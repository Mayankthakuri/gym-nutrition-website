      ...
      <nav className={`sticky top-0 z-50 transition-all duration-300 bg-white ${scrolled ? 'shadow-md' : ''}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 lg:h-20">
            <button onClick={() => setIsOpen(!isOpen)} className="lg:hidden p-2 rounded-lg text-gray-700 hover:bg-gray-100" aria-label="Toggle menu">
              {isOpen ? <X size={24} /> : <Menu size={24} />}
            </button>

            <a href="#home" className="flex items-center gap-2">
              <img src="/logo.svg" alt="BIoton" className="h-8 w-8 lg:h-10 lg:w-10" />
              <span className="text-xl lg:text-2xl font-extrabold text-brand-black tracking-tight">BIOTON</span>
            </a>

            <div className="hidden lg:flex items-center gap-1">
              {navLinks.map((link) => (
                <a key={link.href} href={link.href} className="px-4 py-2 text-sm font-medium text-gray-700 hover:text-brand-orange transition-colors relative group">
                  {link.label}
            ...
            <div className="flex items-center gap-3">
              <button className="hidden sm:flex p-2 text-gray-600 hover:text-brand-orange transition-colors"><Search size={20} /></button>
              <button className="hidden sm:flex p-2 text-gray-600 hover:text-brand-orange transition-colors"><User size={20} /></button>
        ...
        <div className={`lg:hidden overflow-hidden transition-all duration-300 ${isOpen ? 'max-h-80 opacity-100' : 'max-h-0 opacity-0'}`}>
          <div className="bg-white border-t border-gray-100 px-4 py-4 space-y-1">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href} onClick={handleLinkClick} className="block px-4 py-3 text-gray-700 font-medium rounded-lg hover:bg-gray-50 hover:text-brand-orange transition-colors text-sm">
                {link.label}
              </a>
            ))}
            <a href="tel:09558732687" className="flex items-center justify-center gap-2 mt-3 bg-brand-orange text-white px-5 py-3 rounded-full text-sm font-semibold">
              <Phone size={14} />Call Now
            </a>
          </div>
        </div>
