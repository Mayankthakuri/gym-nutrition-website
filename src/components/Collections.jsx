        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {collections.map((col, i) => (
            <a
              key={i}
              href={col.link}
              className={`group relative bg-gradient-to-br ${col.bg} rounded-2xl overflow-hidden aspect-[4/5] flex flex-col justify-end p-6 hover:shadow-xl transition-all duration-300 hover:-translate-y-1`}
            >
              <span className="absolute top-4 left-4 bg-brand-yellow text-brand-black text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                {col.tag}
              </span>
              <img
                src={col.image}
